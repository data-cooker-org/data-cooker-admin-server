...
module.exports = () => {
	class AggExecutor {
		static name = 'AggExecutor';

		static BatchProcessor = (targetData, scriptOnly, logDetails) => {
			// const name = 'BatchProcessor';
				...
				while (batchLoopTag <= batchLoopEnd) {
					...
					if (!scriptOnly) {
						...
						try {
							...
							// });
							// callStmt.execute();
							const SourceResult = this.ChunkProcessor(targetData, scriptOnly, logDetails, nonEnabled, batchLoopTag.toISOString());
		...
		static ChunkProcessor = (targetData, scriptOnly, logDetails, nonEnabled, batchTimeTag) => {
			// const name = 'ChunkProcessor';
